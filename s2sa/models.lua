...
function make_lstm(data, opt, model, use_chars)
  ...
  if use_chars == 0 then
    ...
    input_size = torch.Tensor(opt.num_kernels):sum()
    --input_size = 1000
  end
  local offset = 0
    ...
    if L == 1 then
      if use_chars == 0 then
        ...
        if opt.saved_model=='' then
          char_vecs = nn.LookupTable(data.char_size, opt.char_vec_size)
          char_vecs.name = 'word_vecs' .. name
          x = charcnn(char_vecs(inputs[1]))
          
        else
          local lookup = nn.LinearNoBias( data.char_size , opt.char_vec_size)
          char_vecs = nn.View(1, -1, opt.char_vec_size):setNumInputDims(2)(lookup(OneHot(data.char_size)(inputs[1])))
          lookup.name = 'word_vecs' .. name 
          x  = charcnn(char_vecs) 
...
function make_cnn_2(input_size, kernel_width, num_kernels)
  local output
  num_kernels = 1000
  kernel_width = 6
...
function make_cnn(input_size, kernel_width, num_kernels)
  ...
  local input = nn.Identity()()
  local layer1 = {}
  for i = 1, #kernel_width  do
    local reduced_l = 35 - kernel_width[i] + 1 
    local pool_layer
    if opt.cudnn == 1 then
      local conv = cudnn.SpatialConvolution(1, num_kernels[i], input_size,
        kernel_width[i], 1, 1, 0)
      local conv_layer = conv(nn.View(1, -1, input_size):setNumInputDims(2)(input))
      pool_layer  = nn.Sum(3)(nn.Max(3)(nn.Tanh()(conv_layer)))
      --pool_layer = nn.Squeeze()(cudnn.SpatialMaxPooling(1, reduced_l, 1, 1, 0, 0)(nn.Tanh()(conv_layer)))
    else
      local conv = nn.TemporalConvolution(input_size, num_kernels[i], kernel_width[i])
      local conv_layer = conv(input)
      output = nn.Max(2)(nn.Tanh()(conv_layer))
    end
    table.insert(layer1, pool_layer)
  end
  if #kernel_width > 1 then
