...
function generate_beam(model, K, max_sent_l, source, source_features, gold)
  --reset decoder initial states
  local initial = StateAll.initial(START) 
  ...
  local rnn_state_enc_test = {}
  for i = 1, #init_fwd_enc_test do
    table.insert(rnn_state_enc_test, init_fwd_enc_test[i]:zero())
  end
  local context = context_proto_test[{{}, {1,source_l}}]:clone() -- 1 x source_l x rnn_size
  
  for t = 1, source_l do
    --print(source_input[t]:size())
    local encoder_input = {source_input[t]}
    if opt.num_source_features > 0 then
      append_table(encoder_input, source_features[t])
    end
    append_table(encoder_input, rnn_state_enc_test)
    
    local out = model[1]:forward(encoder_input)
    rnn_state_enc_test = out
    context[{{},t}]:copy(out[#out])
  end
  rnn_state_dec_test = {}
  for i = 1, #init_fwd_dec do
    table.insert(rnn_state_dec_test, init_fwd_dec_test[i]:zero())
  end

  if opt.init_dec == 1 then
    for L = 1, opt.num_layers do 
      rnn_state_dec_test[L*2-1+opt.input_feed]:copy(
        rnn_state_enc_test[L*2-1]:expand(K, opt.rnn_size))
      rnn_state_dec_test[L*2+opt.input_feed]:copy(
        rnn_state_enc_test[L*2]:expand(K, opt.rnn_size))
    end
  end

  if opt.brnn == 1 then
    for i = 1, #rnn_state_enc_test do
      rnn_state_enc_test[i]:zero()
    end
    ...
    if opt.init_dec == 1 then
      for L = 1, opt.num_layers do
        rnn_state_dec_test[L*2-1+opt.input_feed]:add(
          rnn_state_enc_test[L*2-1]:expand(K, opt.rnn_size))
        rnn_state_dec_test[L*2+opt.input_feed]:add(
          rnn_state_enc_test[L*2]:expand(K, opt.rnn_size))
      end
    end
  end
  if opt.score_gold == 1 then
    rnn_state_dec_gold = {}
    for i = 1, #rnn_state_dec_test do
      table.insert(rnn_state_dec_gold, rnn_state_dec_test[i][{{1}}]:clone())
    end
  end
  ...
  while (not done) and (i < n) do
    ...
    local decoder_input
    if opt.attn == 1 then
      decoder_input = {decoder_input1, context, table.unpack(rnn_state_dec_test)}
    else
      decoder_input = {decoder_input1, context[{{}, source_l}], table.unpack(rnn_state_dec_test)}
    end
    --print(decoder_input)
    local out_decoder = model[2]:forward(decoder_input)
    local out = model[3]:forward(out_decoder[#out_decoder]) -- K x vocab_size

    rnn_state_dec_test = {} -- to be modified later
    if opt.input_feed == 1 then
      table.insert(rnn_state_dec_test, out_decoder[#out_decoder])
    end
    for j = 1, #out_decoder - 1 do
      table.insert(rnn_state_dec_test, out_decoder[j])
    end
    ...
    for j = 1, #rnn_state_dec_test do
      rnn_state_dec_test[j]:copy(rnn_state_dec_test[j]:index(1, prev_ks[i]))
    end
    end_hyp = states[i][1]
  ...
  local gold_table
  if opt.rescore ~= '' then
    function last_word(gold_)
      local last = gold_:size(1)+1
      for i=1, gold_:size(1) do
         if gold_[i] == 4 then last=i break end
      end
      return last
    end
    --gold_table = gold[{{2, gold:size(1)-1}}]:totable()
    gold_table = gold[{{2, last_word(gold)-1}}]:totable()
  ...
  if opt.score_gold == 1 then
    rnn_state_dec_test = {}
    for i = 1, #init_fwd_dec do
      table.insert(rnn_state_dec_test, init_fwd_dec_test[i][{{1}}]:zero())
    end
    if opt.init_dec == 1 then
      rnn_state_dec_test = rnn_state_dec_gold
    end
    ...
    for t = 2, target_l do
      ...
      local decoder_input
      if opt.attn == 1 then
        decoder_input = {decoder_input1, context[{{1}}], table.unpack(rnn_state_dec_test)}
      else
        decoder_input = {decoder_input1, context[{{1}, source_l}], table.unpack(rnn_state_dec_test)}
      end
...
--  opt = cmd:parse(arg)
  ...
  opt.simple = 0
  opt.replace_unk = 0
  --opt.model = model_opt.train_from
  opt.targ_file = ''
  opt.num_source_features = 0
  -- some globals
  ...
  checkpoint = opt.model --torch.load(opt.model)
  --print('done!!!')
...
--]]
  ...
  model[2]:apply(get_layer_)
  if model_opt.attn == 1 then
    decoder_attn:apply(get_layer_)
    decoder_softmax = softmax_layers[1]
    attn_layer = torch.zeros(opt.beam, MAX_SENT_L)
  end

  context_proto_test = torch.zeros(1, MAX_SENT_L, model_opt.rnn_size)
  local h_init_dec_test = torch.zeros(opt.beam, model_opt.rnn_size)
  local h_init_enc_test = torch.zeros(1, model_opt.rnn_size)
  if opt.gpuid >= 0 then
    h_init_enc_test = h_init_enc_test:cuda()
    h_init_dec_test = h_init_dec_test:cuda()
    cutorch.setDevice(opt.gpuid)
    if opt.gpuid2 >= 0 then
      cutorch.setDevice(opt.gpuid)
      context_proto_test = context_proto_test:cuda()
      cutorch.setDevice(opt.gpuid2)
  ...
  init_fwd_enc_test = {}
  init_fwd_dec_test = {} -- initial context
  if model_opt.input_feed == 1 then
    table.insert(init_fwd_dec_test, h_init_dec_test:clone())
  end
  
  for L = 1, model_opt.num_layers do
    table.insert(init_fwd_enc_test, h_init_enc_test:clone())
    table.insert(init_fwd_enc_test, h_init_enc_test:clone())
    table.insert(init_fwd_dec_test, h_init_dec_test:clone()) -- memory cell
    table.insert(init_fwd_dec_test, h_init_dec_test:clone()) -- hidden state
  end
...
return {
  init = init,
  --search = search,
  gen_beam = generate_beam,
  --state = StateAll.initial(START),
